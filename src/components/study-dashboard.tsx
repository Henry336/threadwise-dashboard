...
function Work({ study, moduleFilter, onModuleFilter, onEdit, onAdd, onComplete, onArchive }: { study: StudySnapshot; moduleFilter: string; onModuleFilter: (id: string) => void; onEdit: (item: StudyItem) => void; onAdd: () => void; onComplete: (item: StudyItem) => void; onArchive: (item: StudyItem) => void }) {
  ...
  return <section className="study-page">
    <PageHead kicker="Work" title="Work" action={<button className="study-primary" onClick={onAdd}><Plus size={16} /> Add work</button>} />
    ...
    <div className="study-work-list">{visible.map((item) => <article key={item.id} className={item.status === "DONE" ? "done" : ""}><button className="study-work-check" onClick={() => onComplete(item)} aria-label={`Complete ${item.title}`}><Check size={17} /></button><button className="study-work-copy" onClick={() => onEdit(item)}><span><b>{item.module.code}</b><em>{item.publicId}</em><i>{humanize(item.type)}</i>{item.source === "CANVAS" && <i>Canvas</i>}</span><h3>{item.title}</h3><p>{item.notes || attentionCopy(item)}</p></button><div className="study-work-date"><b>{item.dueAt ? formatDate(item.dueAt, study.workspace.timezone) : "No date"}</b><small>{item.dueAt ? formatTime(item.dueAt, study.workspace.timezone) : "Add one when useful"}</small></div><button className="study-row-menu study-archive-action" onClick={() => onArchive(item)} aria-label={`Archive ${item.title}`}><Archive size={17} /></button></article>)}{!visible.length && <Empty title="Nothing in this view." copy="Change the filters or add work." />}</div>
  </section>;
}
...
function PageHead({ kicker, title, copy, action }: { kicker: string; title: string; copy?: string; action?: React.ReactNode }) { return <header className="study-page-head"><div><span>{kicker}</span><h1>{title}</h1>{copy && <p>{copy}</p>}</div>{action}</header>; }
