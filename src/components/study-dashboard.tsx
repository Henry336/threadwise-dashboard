...
import {
  ...
  CheckCircle2, ChevronDown, ChevronRight, CircleHelp, Clock3, Cloud, Download, ExternalLink,
  File, FileText, History, Image as ImageIcon, Library, Link as LinkIcon, ListChecks,
...
import { StudyChoicePicker, StudyFormChoicePicker } from "./study-choice-picker";
import { StudyMarkdown } from "./study-markdown";
import { StudyNoteEditor } from "./study-note-editor";
import { StudyDialog } from "./study-dialog";
import { StudyDeepWork } from "./study-deep-work";
...
import { buildMarkdownExport, markdownExcerpt, normalizeMarkdownWikiTarget, safeMarkdownFileName } from "@/lib/study-markdown";
import { sessionElapsedSeconds, sessionResourceIds } from "@/lib/study-session";
...
export function StudyDashboardApp({ initialData, workspaces, initialView, initialItem, initialItemKind }: Props) {
  ...
  const [openNote, setOpenNote] = useState<StudyResource | null>(null);
  ...
  const openStudyNote = useCallback(async (resourceId: string) => {
    try {
      const detail = await studyApi<{ resource: StudyResource }>(`study/resources/${resourceId}`);
      setOpenNote(detail.resource);
    } catch (error) {
  ...
  return <div className={`study-shell${activeSession ? " focus-active-session" : ""}${desktopSidebarOpen ? "" : " sidebar-collapsed"}`}>
    ...
    <main className="study-main">
      ...
      <div className="study-content" key={view}>
        ...
        {view === "study-overview" && <Overview study={study} ownerName={ownerName} onWriteNote={() => setEditor({ kind: "resource", resourceKind: "NOTE" })} onOpenModule={openModule} onNavigate={navigate} onEditItem={(item) => setEditor({ kind: "item", value: item })} onComplete={(item) => void completeItem(item)} onFocus={(item) => { setFocusItemId(item.id); setModuleFilter(item.moduleId); navigate("study-focus"); }} onTodayChanged={() => void refresh()} />}
        {view === "study-timetable" && <StudyTimetable study={study} busy={busy} onImportNusmods={(url) => mutate("study/nusmods/import", "POST", { url }, "NUSMods timetable imported.")} onConnectCalendar={async () => { const connection = await mutate<{ url: string }>("study/calendar/connect", "POST", {}); if (connection?.url) window.location.assign(connection.url); return connection; }} onSyncCalendar={() => mutate("study/calendar/sync", "POST", {}, "Study timetable synced to Google Calendar.")} onStopCalendar={() => mutate("study/calendar/stop", "POST", {}, "Calendar sync stopped. Existing Google events were left intact.")} onAddBlock={(body) => mutate("study/schedule", "POST", body, "Timetable updated.")} onUpdateBlock={(id, body) => mutate(`study/schedule/${id}`, "PATCH", body, "Timetable updated.")} onDeleteBlock={(id, body) => mutate(`study/schedule/${id}`, "DELETE", body, "Schedule block removed.")} onEditItem={(item) => setEditor({ kind: "item", value: item })} onFocusItem={(item) => { setFocusItemId(item.id); setModuleFilter(item.moduleId); navigate("study-focus"); }} />}
        {view === "study-modules" && <Modules study={study} onOpen={openModule} onEdit={(module) => setEditor({ kind: "module", value: module })} onAdd={() => setEditor({ kind: "module" })} onPin={(module) => void mutate(`study/modules/${module.id}`, "PATCH", { pinned: !module.pinnedAt }, module.pinnedAt ? `${module.code} unpinned.` : `${module.code} pinned.`)} onArchive={(module) => confirmAction(`Archive ${module.code}? Canvas sync will keep it archived until you restore it.`, () => { void (async () => { const saved = await mutate(`study/modules/${module.id}`, "PATCH", { active: false }); if (saved !== undefined) announce(`${module.code} archived.`, "success", { label: "Undo", run: () => void mutate(`study/modules/${module.id}`, "PATCH", { active: true }, `${module.code} restored.`) }); })(); })} onRestore={(module) => void mutate(`study/modules/${module.id}`, "PATCH", { active: true }, `${module.code} restored.`)} />}
...
function Overview({ study, ownerName, onWriteNote, onOpenModule, onNavigate, onEditItem, onComplete, onFocus, onTodayChanged }: { study: StudySnapshot; ownerName: string; onWriteNote: () => void; onOpenModule: (id: string, view?: StudyView) => void; onNavigate: (view: StudyView) => void; onEditItem: (item: StudyItem) => void; onComplete: (item: StudyItem) => void; onFocus: (item: StudyItem) => void; onTodayChanged: () => void }) {
  ...
  return <>
    <section className="study-overview-hero"><div><span>{studyWeekLabel(study)}</span><h1>{greeting()}, {ownerName}.</h1><p>{attention.overdue ? `${attention.overdue} overdue item${attention.overdue === 1 ? "" : "s"} need a decision.` : attention.dueToday ? `${attention.dueToday} item${attention.dueToday === 1 ? "" : "s"} due today.` : "Your semester is in view."}</p></div><button type="button" className="study-overview-note-action" onClick={onWriteNote}><FileText size={17} /> Write note</button></section>
    <TodayPlanner variant="study" modules={study.modules} onChanged={onTodayChanged} />
...
function LibraryView({ study, moduleFilter, activeSession, onModuleFilter, onAdd, onOpenNote, onEdit, onPin, onArchive, onToggleSessionResource }: { study: StudySnapshot; moduleFilter: string; activeSession: StudySession | null; onModuleFilter: (id: string) => void; onAdd: (kind: "NOTE" | "LINK" | "QUESTION") => void; onOpenNote: (resource: StudyResource) => void; onEdit: (resource: StudyResource) => void; onPin: (resource: StudyResource) => void; onArchive: (resource: StudyResource) => void; onToggleSessionResource: (resource: StudyResource) => void }) {
  ...
  return <section className="study-page">
    ...
    <div className="study-toolbar study-library-toolbar"><ModuleSelect modules={study.modules} value={moduleFilter} onChange={onModuleFilter} /><StudyChoicePicker label="Resource type" value={kind} options={[{ value: "ALL", label: "All resources" }, ...(["NOTE", "IMAGE", "LINK", "FILE", "QUESTION"] as StudyResourceKind[]).map((value) => ({ value, label: humanize(value) }))]} allowEmpty={false} onChange={(value) => setKind(value as typeof kind)} /><label><Search size={16} /><span className="sr-only">Search library</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search library" /></label></div>
    <div className="study-resource-grid">{visible.map((resource) => { const caption = imageCaption(resource); const linked = linkedResourceIds.includes(resource.id); const canLink = activeSession?.moduleId === resource.moduleId; const open = () => resource.kind === "NOTE" ? onOpenNote(resource) : onEdit(resource); return <article key={resource.id} className={`kind-${resource.kind.toLowerCase()}`}><header><span>{resourceIcon(resource.kind)}{resource.module.code}</span><div>{resource.pinnedAt && <Pin size={14} />}{canLink && <button className={`study-resource-session-link${linked ? " active" : ""}`} aria-pressed={linked} onClick={() => onToggleSessionResource(resource)} title={linked ? "Remove from active session" : "Use in active session"} aria-label={linked ? "Remove from active session" : "Use in active session"}><LinkIcon size={16} /></button>}<button onClick={() => onPin(resource)} aria-label={resource.pinnedAt ? "Unpin resource" : "Pin resource"}><Pin size={16} /></button><button className="study-archive-action" onClick={() => onArchive(resource)} aria-label="Archive resource"><Archive size={16} /></button></div></header>{resource.kind === "IMAGE" ? <><button className="study-resource-image" onClick={() => setOpenImage(resource)} aria-label={`View ${caption || "saved image"}`}><img src={`/api/threadwise/study/resources/${resource.id}/content`} alt={caption || `Saved image for ${resource.module.code}`} /></button>{caption && <button className="study-resource-copy study-image-caption" onClick={() => setOpenImage(resource)}><p>{caption}</p></button>}</> : <button className="study-resource-copy" onClick={open}><h3>{resource.title}</h3><p>{resource.kind === "NOTE" ? markdownExcerpt(resource.body ?? "") : resource.body || resource.caption || resource.url || resource.fileName || "Open resource"}</p></button>}<footer><span>{resource.publicId}</span>{resource.kind === "NOTE" && <i>Markdown</i>}{linked && <i>In session</i>}{resource.kind === "IMAGE" && <button className="study-resource-open" onClick={() => setOpenImage(resource)}>View <ExternalLink size={13} /></button>}{resource.kind === "FILE" && <a href={`/api/threadwise/study/resources/${resource.id}/content`} target="_blank" rel="noreferrer">Open <ExternalLink size={13} /></a>}</footer></article>; })}{!visible.length && <Empty title="No resources here yet." copy="Capture one in Telegram or add a note, link, or question here." />}</div>
...
function StudyNoteReader({ resource, timezone, onClose, onEdit, onOpenLinked }: { resource: StudyResource; timezone: string; onClose: () => void; onEdit: (resource: StudyResource) => void; onOpenLinked: (target: string) => void }) {
  const metadata = resource.noteMeta;
  const download = () => {
    const markdown = buildMarkdownExport({ title: resource.title, body: resource.body ?? "", moduleCode: resource.module.code, publicId: resource.publicId });
    downloadTextFile(safeMarkdownFileName(resource.title), markdown, "text/markdown;charset=utf-8");
  };
  return <StudyDialog kicker={`${resource.module.code} · ${resource.publicId}`} title={resource.title} wide onClose={onClose}>{(requestClose) =>
    <div className="study-note-reader">
      <div className="study-note-reader-actions"><span>Updated {formatDate(resource.updatedAt, timezone)}</span><button type="button" className="study-secondary" onClick={download}><Download size={15} /> Export .md</button><button type="button" className="study-primary" onClick={() => onEdit(resource)}><FileText size={15} /> Edit note</button></div>
      <div className="study-note-reader-layout">
        <article className="study-note-document"><StudyMarkdown source={resource.body ?? ""} onOpenNote={onOpenLinked} /></article>
        <aside className="study-note-context">
          <section><span>Connections</span><h3>Linked notes</h3>{metadata?.outgoingLinks.length ? <div>{metadata.outgoingLinks.map((link) => <button type="button" key={`${link.target}-${link.label}`} disabled={!link.resolved} onClick={() => onOpenLinked(link.resource?.id ?? link.target)}><LinkIcon size={14} /><span><b>{link.label}</b><small>{link.resolved ? link.resource?.moduleCode : "Not created yet"}</small></span></button>)}</div> : <p>Write <code>[[Note title]]</code> to connect another note.</p>}</section>
          <section><span>Backlinks</span><h3>Notes pointing here</h3>{metadata?.backlinks.length ? <div>{metadata.backlinks.map((link) => <button type="button" key={link.id} onClick={() => onOpenLinked(link.publicId)}><ArrowRight size={14} /><span><b>{link.title}</b><small>{link.moduleCode} · {link.publicId}</small></span></button>)}</div> : <p>No other notes point here yet.</p>}</section>
          <details className="study-note-history"><summary><History size={15} /> Version history <span>{metadata?.revisions.length ?? 0}</span></summary><div>{metadata?.revisions.map((revision, index) => <article key={revision.id}><div><b>{index === 0 ? "Latest saved version" : formatDate(revision.createdAt, timezone)}</b><small>{humanize(revision.source)}</small></div><p>{markdownExcerpt(revision.body, 120)}</p><button type="button" onClick={() => onEdit({ ...resource, title: revision.title, body: revision.body })}>Use in editor</button></article>)}{!metadata?.revisions.length && <p>History begins with the next saved version.</p>}</div></details>
        </aside>
      </div>
      <footer><button type="button" className="study-secondary" onClick={requestClose}>Close</button></footer>
    </div>
  }</StudyDialog>;
}
  ...
  return <div className={`study-image-lightbox${expanded ? " is-expanded" : ""}`} role="dialog" aria-modal="true" aria-labelledby="study-image-title" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <article>
      ...
      <div className="study-image-context">
        {caption && <p>{caption}</p>}
        <div className="study-image-meta"><span>{resource.module.code}</span><span>{formatDate(resource.createdAt, timezone)}</span></div>
      ...
      <div className="study-settings-panel">
        ...
        {panel === "rhythm" && <section aria-labelledby="study-settings-rhythm">
          ...
          <form onSubmit={(event) => { event.preventDefault(); void onSave(studySettingsPayload(settings)); }}>
            ...
            <fieldset><legend>Weekly rhythm</legend><div className="study-form-row"><StudyChoicePicker label="Preview day" value={String(settings.weeklyPreviewDay)} options={[1,2,3,4,5,6,7].map((value) => ({ value: String(value), label: weekday(value) }))} allowEmpty={false} onChange={(value) => setSettings({ ...settings, weeklyPreviewDay: Number(value) })} /><label>Preview time<input type="time" value={settings.weeklyPreviewTime} onChange={(event) => setSettings({ ...settings, weeklyPreviewTime: event.target.value })} /></label><StudyChoicePicker label="Review day" value={String(settings.weeklyReviewDay)} options={[1,2,3,4,5,6,7].map((value) => ({ value: String(value), label: weekday(value) }))} allowEmpty={false} onChange={(value) => setSettings({ ...settings, weeklyReviewDay: Number(value) })} /><label>Review time<input type="time" value={settings.weeklyReviewTime} onChange={(event) => setSettings({ ...settings, weeklyReviewTime: event.target.value })} /></label></div></fieldset>
...
function ScheduleSettings({ study, busy, onAdd, onUpdate, onDelete }: { study: StudySnapshot; busy: boolean; onAdd: (body: unknown) => Promise<unknown>; onUpdate: (id: string, body: unknown) => Promise<unknown>; onDelete: (id: string) => Promise<unknown> }) {
  ...
  return <section aria-labelledby="study-settings-schedule">
    ...
    <form className="study-block-form study-labelled-form" onSubmit={(event) => { event.preventDefault(); void onAdd({ moduleId: moduleId || undefined, dayOfWeek: day, startTime: start, endTime: end, label, destination: destination || undefined, destinationPlaceId: destinationPlaceId || undefined, defaultOriginId: originId || undefined, travelBufferMinutes: clampInteger(buffer, 0, 90) }).then((saved) => { if (saved) { setLabel(""); setDestination(""); setDestinationPlaceId(null); } }); }}>
      <StudyChoicePicker label="Module" value={moduleId} placeholder="No module" options={study.modules.map((module) => ({ value: module.id, label: module.code, detail: module.name }))} searchable onChange={setModuleId} />
      <StudyChoicePicker label="Day" value={String(day)} options={[1,2,3,4,5,6,7].map((value) => ({ value: String(value), label: weekday(value) }))} allowEmpty={false} onChange={(value) => setDay(Number(value))} />
      ...
      <StudyChoicePicker label="Usual origin" value={originId} placeholder="Current / default" options={study.origins.map((origin) => ({ value: origin.id, label: origin.name }))} searchable onChange={setOriginId} />
...
function ScheduleTravelEditor({ block, origins, busy, onCancel, onSave }: { block: StudySnapshot["scheduleBlocks"][number]; origins: StudySnapshot["origins"]; busy: boolean; onCancel: () => void; onSave: (body: unknown) => Promise<void> }) {
  ...
  return <form className="study-travel-editor" onSubmit={(event) => { event.preventDefault(); void onSave({ destination: destination || null, destinationPlaceId, defaultOriginId: originId || null, travelBufferMinutes: clampInteger(buffer, 0, 90) }); }}>
    ...
    <StudyChoicePicker label="Usual origin" value={originId} placeholder="Current / default" options={origins.map((origin) => ({ value: origin.id, label: origin.name }))} searchable onChange={setOriginId} />
...
function StudyEditor({ state, study, busy, onClose, onSave }: { state: Exclude<Editor, null>; study: StudySnapshot; busy: boolean; onClose: () => void; onSave: (path: string, method: "POST" | "PATCH", body: unknown, message: string) => Promise<boolean> }) {
  ...
  if (state.kind === "resource" && (state.value?.kind ?? state.resourceKind ?? "NOTE") === "NOTE") {
    return <StudyNoteEditor value={state.value} study={study} busy={busy} onClose={onClose} onSave={onSave} />;
  }
  const submit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (state.kind === "resource") {
      ...
      void onSave(value ? `study/resources/${value.id}` : "study/resources", value ? "PATCH" : "POST", { moduleId: formText(form, "moduleId"), ...(value ? {} : { kind }), title: formText(form, "title"), body: formText(form, "body") || undefined, url: formText(form, "url") || undefined, caption: formText(form, "caption") || undefined }, value ? "Resource updated." : "Resource saved.");
    }
  ...
  return <StudyDialog kicker="Study Mode" title={heading} dirty={dirty} onClose={onClose}>{(requestClose) =>
    <form onSubmit={submit} onChange={() => setDirty(true)}>
      ...
      {state.kind === "item" && <><div className="study-form-row"><ModuleField modules={study.modules} name="moduleId" value={state.value?.moduleId ?? study.workspace.activeModuleId ?? study.modules[0]?.id} /><StudyFormChoicePicker label="Type" name="type" defaultValue={state.value?.type ?? "ASSIGNMENT"} options={ITEM_TYPES.map((type) => ({ value: type, label: humanize(type) }))} allowEmpty={false} /></div><label>Title<input name="title" required maxLength={500} defaultValue={state.value?.title ?? ""} /></label><label>Details<textarea name="notes" rows={5} defaultValue={state.value?.notes ?? ""} /></label><div className="study-form-row"><label>Due date<input name="dueAt" type="datetime-local" defaultValue={localInput(state.value?.dueAt)} /></label><label>Planned minutes<input name="plannedMinutes" type="number" min="1" max="1440" defaultValue={state.value?.plannedMinutes ?? ""} /></label></div><div className="study-form-row"><StudyFormChoicePicker label="Priority" name="priority" defaultValue={state.value?.priority ?? "NORMAL"} options={["LOW", "NORMAL", "HIGH", "CRITICAL"].map((value) => ({ value, label: humanize(value) }))} allowEmpty={false} />{state.value && <StudyFormChoicePicker label="Status" name="status" defaultValue={state.value.status} options={["OPEN", "IN_PROGRESS", "PROCESSED", "DONE"].map((value) => ({ value, label: humanize(value) }))} allowEmpty={false} />}</div></>}
      ...
      {state.kind === "mistake" && <><input type="hidden" name="itemId" value={state.item?.id ?? ""} /><ModuleField modules={study.modules} name="moduleId" value={mistakeModuleId} />{state.item && <div className="study-linked-context"><Target size={17} /><div><small>Linked work</small><b>{state.item.title}</b></div></div>}<label>Question or source<textarea name="source" required rows={3} defaultValue={state.item?.title ?? ""} /></label><StudyFormChoicePicker label="Category" name="category" defaultValue="CONCEPTUAL_MISUNDERSTANDING" options={[{ value: "CONCEPTUAL_MISUNDERSTANDING", label: "Conceptual misunderstanding" }, { value: "WRONG_APPROACH", label: "Wrong approach" }, { value: "EXECUTION_CARELESS", label: "Execution / careless" }, { value: "TIME_MANAGEMENT", label: "Time management" }]} allowEmpty={false} /><label>Cause<textarea name="cause" required rows={3} /></label><label>Preventive check<textarea name="prevention" required rows={3} /></label><label>Revisit<input name="revisitAt" type="datetime-local" /></label></>}
...
function ModuleSelect({ modules, value, onChange }: { modules: StudyModule[]; value: string; onChange: (value: string) => void }) { return <StudyChoicePicker label="Module" value={value} placeholder="All modules" options={modules.map((module) => ({ value: module.id, label: module.code, detail: module.name }))} searchable onChange={onChange} />; }
function ModuleField({ modules, name, value }: { modules: StudyModule[]; name: string; value?: string | null }) { return <StudyFormChoicePicker label="Module" name={name} required defaultValue={value ?? modules[0]?.id} options={modules.map((module) => ({ value: module.id, label: module.code, detail: module.name }))} allowEmpty={false} searchable />; }
