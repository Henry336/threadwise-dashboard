...
function StudySettings({ study, busy, onSave, onSync, onCanvasReview, onAddOrigin, onOrigin, onDeleteOrigin, onAddBlock, onDeleteBlock }: { study: StudySnapshot; busy: boolean; onSave: (body: unknown) => Promise<unknown>; onSync: () => Promise<unknown>; onCanvasReview: (id: string, action: "keep" | "archive") => Promise<unknown>; onAddOrigin: (body: unknown) => Promise<unknown>; onOrigin: (id: string, body: unknown) => Promise<unknown>; onDeleteOrigin: (id: string) => Promise<unknown>; onAddBlock: (body: unknown) => Promise<unknown>; onDeleteBlock: (id: string) => Promise<unknown> }) {
  ...
  return <section className="study-page">
    ...
    <div className="study-settings-layout">
      ...
      <div className="study-settings-panel">
        ...
        {panel === "schedule" && <ScheduleSettings study={study} busy={busy} onAdd={onAddBlock} onDelete={onDeleteBlock} />}
      </div>
    </div>
  </section>;
}

function ScheduleSettings({ study, busy, onAdd, onDelete }: { study: StudySnapshot; busy: boolean; onAdd: (body: unknown) => Promise<unknown>; onDelete: (id: string) => Promise<unknown> }) {
  ...
  return <section aria-labelledby="study-settings-schedule">
    <header><div><span>Schedule</span><h2 id="study-settings-schedule">Recurring study time</h2><p>Create reliable blocks without filling the calendar one week at a time.</p></div></header>
    {study.scheduleBlocks.length === 0 ? <Empty title="No recurring blocks" copy="Add the first study block below." /> : <div className="study-block-list">{study.scheduleBlocks.map((block) => <article key={block.id}><Clock3 size={17} /><div><b>{block.label}</b><small>{weekday(block.dayOfWeek)} · {block.startTime}–{block.endTime}{block.module ? ` · ${block.module.code}` : ""}</small></div><button aria-label={`Remove ${block.label}`} onClick={() => confirmAction(`Remove ${block.label}?`, () => onDelete(block.id))}><Trash2 size={15} /></button></article>)}</div>}
    <form className="study-block-form study-labelled-form" onSubmit={(event) => { event.preventDefault(); void onAdd({ moduleId: moduleId || undefined, dayOfWeek: day, startTime: start, endTime: end, label }).then((saved) => { if (saved) setLabel(""); }); }}>
      ...
      <label>Label<input value={label} onChange={(event) => setLabel(event.target.value)} placeholder="Algorithms review" required /></label>
