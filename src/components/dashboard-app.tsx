    ...
    <div className="tw-settings-panel">
      ...
      {tab === "reminders" && <>
        <header className="tw-settings-ari-header">
          <div><span>Reminders</span><h2>Helpful, never noisy</h2><p>These controls govern bot reminders as well as dashboard-created tasks.</p></div>
          <Ari variant="avatar-light" className="tw-settings-ari" decorative />
        </header>
        <form onSubmit={save}>
          <div className="tw-form-row">
            <label>Default rhythm<select value={settings.reminderIntervalMinutes} onChange={(event) => setSettings({ ...settings, reminderIntervalMinutes: Number(event.target.value) })}><option value="60">Every hour</option><option value="180">Every 3 hours</option><option value="360">Every 6 hours</option><option value="1440">Daily</option></select></label>
            <label>Delivery style<select value={settings.reminderMode} onChange={(event) => setSettings({ ...settings, reminderMode: event.target.value as DashboardSettings["reminderMode"] })}><option value="INDIVIDUAL">Individual cards</option><option value="DIGEST">Compact digest</option></select></label>
          </div>
          <div className="tw-form-row">
            <label>
