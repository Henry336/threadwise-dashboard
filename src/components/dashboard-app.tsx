      ...
      {tab === "reminders" && <>
        ...
        <form onSubmit={save}>
          <div className="tw-form-row">
            <label>Regular follow-up frequency<select value={settings.reminderIntervalMinutes} onChange={(event) => setSettings({ ...settings, reminderIntervalMinutes: Number(event.target.value) })}><option value="60">Every hour</option><option value="180">Every 3 hours</option><option value="360">Every 6 hours</option><option value="1440">Daily</option></select><small>Used for open tasks before their deadline-warning window, and for tasks without a deadline.</small></label>
            <label>Message format<select value={settings.reminderMode} onChange={(event) => setSettings({ ...settings, reminderMode: event.target.value as DashboardSettings["reminderMode"] })}><option value="INDIVIDUAL">One card per task</option><option value="DIGEST">Group reminders into a digest</option></select><small>Choose separate task cards or one shorter message containing several reminders.</small></label>
          </div>
          <div className="tw-form-row">
            <label>
...
function EntityEditor({ state, busy, currency, timezone, onClose, onSave, onDelete, onConvert }: { state: EditorState; busy: boolean; currency: string; timezone: string; onClose: () => void; onSave: (kind: EditableKind, values: Record<string, unknown>, item?: EditorState["item"]) => void; onDelete: (kind: EditableKind, item: NonNullable<EditorState["item"]>) => void; onConvert: (idea: DashboardIdea) => void }) {
  ...
  return <div className="tw-modal-overlay" onMouseDown={onClose}><section ref={dialogRef} className="tw-sheet" role="dialog" aria-modal="true" aria-label={title} onMouseDown={(event) => event.stopPropagation()}><header><div><span>{state.kind}</span><h2>{title}</h2></div><button onClick={onClose} aria-label="Close"><X size={20} /></button></header><form onSubmit={submit}>
    ...
    {state.kind === "idea" && <><label>Idea title<input name="title" required maxLength={500} autoFocus defaultValue={(item as DashboardIdea | undefined)?.title ?? state.seed ?? ""} /></label><label>Concept<textarea name="concept" required maxLength={20000} rows={7} defaultValue={(item as DashboardIdea | undefined)?.concept ?? state.seed ?? ""} /></label><label>Stage<select name="status" defaultValue={(item as DashboardIdea | undefined)?.status ?? "RAW"}>{IDEA_STATUSES.map((status) => <option key={status} value={status}>{status.toLowerCase()}</option>)}</select></label><label className="tw-checkbox"><input type="checkbox" name="pinned" defaultChecked={(item as DashboardIdea | undefined)?.pinned} /> Pin this idea</label></>}
