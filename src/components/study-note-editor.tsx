...
export function StudyNoteEditor({ value, study, busy, onClose, onSave }: { value?: StudyResource; study: StudySnapshot; busy: boolean; onClose: () => void; onSave: (path: string, method: "POST" | "PATCH", body: unknown, message: string) => Promise<boolean> }) {
  ...
  const loadDraft = useCallback(async () => {
    ...
    try {
      ...
      const response = await noteApi<{ draft: RemoteDraft | null }>(`study/note-drafts${query}`);
  ...
  const persistDraft = useCallback((snapshot = draftRef.current) => {
    ...
    const queued = saveQueueRef.current.then(async () => {
      ...
      setMessage("");
      try {
        const response = await noteApi<{ draft: RemoteDraft }>("study/note-drafts", "PATCH", {
  ...
  const fileNote = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try { await noteApi(`study/note-drafts/${draftIdRef.current}`, "DELETE", {}); } catch { /* A stale draft is safe and expires automatically. */ }
  };
  ...
  return createPortal(<div className="study-note-overlay" onMouseDown={(event) => { if (event.target === event.currentTarget) void requestClose(); }}>
    <section ref={modalRef} className="study-note-fullscreen" role="dialog" aria-modal="true" aria-labelledby="study-note-editor-title">
...
async function noteApi<T>(path: string, method = "GET", body?: unknown): Promise<T> {
