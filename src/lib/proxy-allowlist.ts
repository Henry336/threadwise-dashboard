...
export function isAllowedThreadwiseProxyMethod(method: string, path: string) {
  ...
  if (path === "note-drafts") return method === "GET" || method === "PATCH";
  if (new RegExp(`^note-drafts/${UUID_SEGMENT}$`).test(path)) return method === "DELETE";
