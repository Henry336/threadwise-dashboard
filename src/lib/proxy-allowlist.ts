const STUDY_PATH = /^study\/(?:snapshot|search|modules(?:\/[A-Za-z0-9_-]+)?|items(?:\/[A-Za-z0-9_-]+(?:\/complete)?)?|resources(?:\/[A-Za-z0-9_-]+(?:\/content)?)?|sessions\/(?:start|stop|[A-Za-z0-9_-]+)|mistakes(?:\/[A-Za-z0-9_-]+\/resolve)?|weekly-plan|review|settings|canvas\/sync|canvas\/assignments\/[A-Za-z0-9_-]+|origins(?:\/[A-Za-z0-9_-]+)?|schedule(?:\/[A-Za-z0-9_-]+)?)$/;
...
export function isAllowedThreadwiseProxyMethod(method: string, path: string) {
  if (path === "study/snapshot" || path === "study/search" || /^study\/resources\/[A-Za-z0-9_-]+\/content$/.test(path)) return method === "GET";
